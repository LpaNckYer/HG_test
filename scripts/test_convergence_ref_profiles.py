...
用法示例：
  ...
  python scripts/test_convergence_ref_profiles.py --initial-mesh-up 500 --initial-mesh-down 100

日志 / 汇总：同原脚本，`--log-bvp`、`--log-hc`、`--output-csv`；BVP/HC 在各自子目录下运行。
...
# ---------------------------------------------------------------------------
DEFAULT_BVP_UP = ROOT / "data" / "initial_case_bvp_0.0-4.2m_loop.csv"
DEFAULT_BVP_DOWN = ROOT / "data" / "initial_case_DOWN_bvp_4.2-5.9m_loop.csv"
DEFAULT_HC_UP = ROOT / "data" / "test_hc_4n4_1e-3_UP_loop.csv"
DEFAULT_HC_DOWN_PRIMARY = ROOT / "data" / "test_hc_4n4_1e-3_DOWN_loop.csv"
DEFAULT_HC_DOWN_FALLBACK = ROOT / "data" / "test_hc_6_1e-3_DOWN_loop.csv"
...
def _configure_hybrid_logging(log_path: Path, *, append: bool = True) -> None:
...
def bind_initial_bvp_guess_down(params, y_stack: np.ndarray) -> None:
    H_ctrl = [params.H0, params.H1, params.HH]
...
def run_hegang_bvp_coupled(
    ...
    *,
    ...
    coupling_ok：结束时 |y_new - y_in| <= y_tol 且无异常。
...
def run_hegang_hc_coupled(
    ...
    *,
    ...
    """与 hegang_hc.py 相同：test_hc_4n4 + test_hc_6 + 风口迭代。profile_out_dir 成功时写出 HC 剖面 CSV。"""
    bind_initial_bvp_guess_up(params_up, y_up)
    bind_initial_bvp_guess_down(params_down, y_down)
...
def main() -> None:
    ...
    for i, scale in enumerate(scale_list):
        ...
        # ---------- BVP ----------
        _configure_hybrid_logging(args.log_bvp)
        logging.info("======== 试验 %d/%d scale=%s ======== BVP", i + 1, len(scale_list), scale)
        ...
        logging.info(
            "BVP 结束: coupling_ok=%s, outer_iters=%d, elapsed=%.2fs",
            bvp_ok,
            bvp_outer,
            elapsed_bvp,
        )
        ...
        # ---------- HC ----------
        _configure_hybrid_logging(args.log_hc)
        logging.info("======== 试验 %d/%d scale=%s ======== HC", i + 1, len(scale_list), scale)
        ...
        logging.info(
            "HC 结束: coupling_ok=%s, outer_iters=%d, elapsed=%.2fs",
            hc_ok,
            hc_outer,
            elapsed_hc,
        )
        ...
        print(
            ...
            f"  BVP: coupling_ok={bvp_ok}, outer={bvp_outer}, {elapsed_bvp:.2f}s\n"
            f"  HC: coupling_ok={hc_ok}, outer={hc_outer}, {elapsed_hc:.2f}s\n"
            f"  overall_success={overall}"
        )

    pd.DataFrame(results_rows).to_csv(args.output_csv, index=False)
    print(f"汇总已写入: {args.output_csv} （共 {len(results_rows)} 行）")
